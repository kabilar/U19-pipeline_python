...
class RecProcessHandler:
    def pipeline_handler_main():
        ...
        # For all active process jobs
            ...
            if rec_process_series["recording_modality"] == "ephys":
                ...
                precluster_param_list_id, paramset_idx = (
                    recording_process.Processing.EphysParams & rec_process_series
                ).fetch1("precluster_param_list_id", "paramset_idx")

                precluster_param_list = (
                    ephys_element.PreClusterParamList.ParamOrder
                    & f"precluster_param_list_id={precluster_param_list_id}"
                ).fetch()
                ...
                rec_process_series["preprocess_paramset"] = []
                    ...
                    params = (
                        ...
                        & f'paramset_idx={precluster_param["paramset_idx"]}'
                    ).fetch1("params")

                    rec_process_series["preprocess_paramset"].append(
                        dict(order_id=precluster_param["order_id"], params=params)
                    )

                rec_process_series["process_paramset"] = (
                    ephys_element.ClusteringParamSet & f"paramset_idx={paramset_idx}"
                ).fetch1("params")

            elif rec_process_series["recording_modality"] == "imaging":

                paramset_idx = (
                    recording_process.Processing.ImagingParams & rec_process_series
                ).fetch1("paramset_idx")

                rec_process_series["process_paramset"] = (
                    imaging_element.ProcessingParamSet & f"paramset_idx={paramset_idx}"
                ).fetch1("params")
    ...
    def slurm_job_queue(rec_series, status_series):
        ...
        if status == config.system_process["SUCCESS"]:
            ...
            update_value_dict["error_info"][
                "error_message"
            ] = "Error while generating/transfering pereparameterfile file"
    ...
    def get_active_process_jobs():
        ...
        status_query = "status_processing_id > " + str(
            config.recording_process_status_df["Value"].min()
        )
        status_query += " and status_processing_id < " + str(
            config.recording_process_status_df["Value"].max()
        )
        ...
        if df_process_jobs.shape[0] > 0:
            ...
            df_process_jobs = df_process_jobs.loc[
                df_process_jobs["recording_modality"] == "electrophysiology", :
            ].copy()
